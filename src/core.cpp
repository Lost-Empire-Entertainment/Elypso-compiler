...
namespace Core
{
	void Compiler::MainInitialize()
	{
		...
		if (SUCCEEDED(result))
		{
			...
			docsPath = (path(narrowPath) / "Elypso compiler").string();
		}
		else
		{
		...
		docsPath = path(documentsFolder) / "Elypso compiler";
#endif

		if (!exists(docsPath)) File::CreateNewFolder(docsPath);

		cout << "Elypso compiler documents path: " << docsPath << "\n";
	...
	{
		string title = "Elypso compiler has shut down";
	...
	{
		cout << "Shutting down Elypso compiler...\n";
